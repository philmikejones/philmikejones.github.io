...
For a project I’m working on I needed to compare three figures each for
...
To do this I wrote the following small python script using the
`pillow <https://python-pillow.org/>`__ library. The script:

1. creates a blank canvas (A3 size),
2. loads the images,
3. places them together on a blank canvas, and
4. saves the output for review.

My images were called:

-  ``mort_ABC.png`` (``mort_`` means mortality; ``ABC`` are `alpha–3
   country
   codes <https://en.wikipedia.org/wiki/ISO_3166-1#Current_codes>`__) is
   a Lexis surface plot of mortality, one for each country; and
-  ``cohort_ABC.png`` are line charts, one for females and one for males
   for each country.

``mort_ABC.png`` is about 420mm x 148mm, and fills the top half of each
