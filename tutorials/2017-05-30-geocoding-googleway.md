...
![Google Geocoding set up](../_static/img/enable-geocoding-platform.png)
...
One way to use your API key is to store it in a text file that is not kept with your data analysis.
That way you can still share the data analysis script or results without sharing the key.

For example, add `keys/` to `.gitignore` so that folder isn't tracked and shared, and place your key in there.
Load it with `readLines()`.

Alternatively set an environment variable with your key, for example `GEOCODE_TUTORIAL_KEY` which you can then load in to your R session with `Sys.getenv`:
