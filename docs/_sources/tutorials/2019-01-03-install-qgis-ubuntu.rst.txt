...
Each new version of Ubuntu usually has the most up–to–date libraries
...
are also useful for using ``sf`` and related libraries.

To install ``ubuntugis-unstable`` follow the instructions at
...
Don’t worry about the ``unstable`` in the name. It doesn’t mean it will
...
downloads so that nothing malicious is installed on your system.

Under the `Debian/Ubuntu installation
...
Enter these commands in your terminal. The ``wget`` line downloads the
...
You might need to enter your password again, then copy and paste the
following three lines to the bottom of this file:
...
This will install the basic packages needed and you should now be able
to run QGIS! You might need to install additional libraries later, but
these can be installed as necessary.
